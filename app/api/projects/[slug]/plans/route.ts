...
export async function GET(_req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const { data, error } = await supabaseAdmin().from('project_site_plans').select('id,project_id,plan_type,map_url,drone_url,created_at,updated_at').eq('project_id', project.id).order('plan_type');
    if (error) throw error;
...
export async function POST(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const { data, error } = await supabaseAdmin().from('project_site_plans').upsert({ project_id: access.project.id, plan_type: planType }, { onConflict: 'project_id,plan_type' }).select('id,project_id,plan_type,map_url,drone_url,created_at,updated_at').single();
    if (error) throw error;
...
export async function DELETE(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    if (planType === 'master_plan') return NextResponse.json({ error: 'Master Plan cannot be deleted.' }, { status: 400 });
    if (!/^section_\d+$/.test(planType)) return NextResponse.json({ error: 'Only section_N plans can be deleted.' }, { status: 400 });
    const { error } = await supabaseAdmin().from('project_site_plans').delete().eq('project_id', access.project.id).eq('plan_type', planType);
    if (error) throw error;
    return NextResponse.json({ ok: true, planType });
  } catch (error) {
    console.error('project plans DELETE failed', error);
    return NextResponse.json({ error: 'Unable to delete project section.' }, { status: 500 });
  }
}
