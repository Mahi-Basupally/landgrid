...
export default async function ManageProject({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: project, error } = await supabaseAdmin().from('projects').select('id,slug,name,address,created_by').eq('slug', slug).maybeSingle();
  ...
  return <main className="shell">
    ...
    <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) 290px',gap:20,alignItems:'start'}}>
      <section><ProjectPlanViewer projectName={project.name} projectSlug={project.slug} editable={false}/></section>
      <aside className="details"><div className="eyebrow">PROJECT</div><h2>Manage plots</h2><p>View the master plan, sections and plots. Map and drone assets are uploaded and aligned from the Editor.</p><div style={{display:'grid',gap:10,marginTop:20}}><Link className="button primary full" href={`/projects/${slug}/editor`}>Open Editor</Link><Link className="button secondary full" href={`/projects/${slug}`}>View public project</Link>{role==='admin'&&<Link className="button secondary full" href={`/projects/${slug}/settings`}>Project settings</Link>}</div></aside>
    </div>
  </main>;
}
