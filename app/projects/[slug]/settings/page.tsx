...
export default async function ProjectSettingsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><style>{`.app-shell > div[style*="grid-template-rows: 68px 1fr"]{height:calc(100vh - 64px)!important;min-height:0!important;overflow:hidden!important}.app-shell > div[style*="grid-template-rows: 68px 1fr"] > div{min-height:0!important}.app-shell > div[style*="grid-template-rows: 68px 1fr"] main{min-height:0}`}</style><SettingsClient project={project} members={members} /></>;
}
