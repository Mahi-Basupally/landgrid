...
export default async function ProjectSettingsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: exactProject } = await db.from('projects').select('id,slug,name,address,description,google_location_url,site_plan_url,drone_url,created_by,created_at').eq('slug', requestedSlug).maybeSingle();
  let project = exactProject;
  if (!project) {
    const { data: projects, error } = await db.from('projects').select('id,slug,name,address,description,google_location_url,site_plan_url,drone_url,created_by,created_at');
  ...
  return (
    <main className="shell">
      ...
      <section className="page-heading"><div className="eyebrow">PROJECT SETTINGS</div><h1>{project.name}</h1><p>Configure the project, plan, drone view, media and public presentation.</p></section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '220px minmax(0,1fr)' }}>
          <aside style={{ padding: 18, borderRight: '1px solid var(--border, #e5e7eb)' }}>
            ...
            <div style={{ display: 'grid', gap: 6, marginTop: 12 }}>
              <a href="#project" className="button secondary">Project</a><a href="#plan" className="button secondary">Project plan</a><a href="#media" className="button secondary">Media</a><a href="#owner" className="button secondary">Owner</a><a href={`/projects/${project.slug}/members`} className="button secondary">Project users</a>
            </div>
          </aside>
          <section style={{ padding: 24, minWidth: 0 }}>
            <form id="settings-form" action={`/api/projects/${project.slug}/settings`} method="post" style={{ display: 'grid', gap: 24 }}>
              ...
              <section id="plan"><div className="eyebrow">2 · PROJECT PLAN</div><h2>Master plan &amp; drone map</h2><p>Upload the assets once. They are stored in Supabase Storage and survive Vercel redeployments.</p><div style={{ display: 'grid', gap: 18, marginTop: 14 }}><div><label>Master/site plan SVG or image</label><ProjectAssetUpload slug={project.slug} kind="master-plan" accept="image/svg+xml,image/png,image/jpeg,image/webp" currentUrl={project.site_plan_url} /></div><div><label>Drone map image or SVG</label><ProjectAssetUpload slug={project.slug} kind="drone" accept="image/svg+xml,image/png,image/jpeg,image/webp" currentUrl={project.drone_url} /></div><div><label>Sections</label><p style={{ marginTop: 4 }}>Section-specific assets can use the same persistent project storage.</p></div></div></section>
