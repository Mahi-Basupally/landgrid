...
export default function CapeTownSettingsPage() {
  ...
  async function load() {
    try {
      const [projectsResponse, membersResponse] = await Promise.all([fetch("/api/projects", { cache: "no-store" }), fetch("/api/projects/cape-town/members", { cache: "no-store" })]);
  ...
  async function addMember(e: FormEvent) {
    ...
    try {
      ...
      setEmail(""); setMessage("User added"); await load();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to add user"); }
    finally { setBusy(false); }
  }

  if (!project) return <div style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "#f4f6f9", color: "#64748b", fontFamily: "Inter,ui-sans-serif,system-ui,sans-serif" }}>Loading project settings…</div>;

  return <div style={{ minHeight: "100vh", background: "#f4f6f9", color: "#182235", fontFamily: "Inter,ui-sans-serif,system-ui,sans-serif" }}>
    <header style={{ height: 64, boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", background: "rgba(255,255,255,.98)", borderBottom: "1px solid #e2e8f0", position: "sticky", top: 0, zIndex: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ width: 34, height: 34, borderRadius: 9, display: "grid", placeItems: "center", background: "#172554", color: "#fff", fontWeight: 900 }}>LG</div><div><div style={{ fontSize: 10, letterSpacing: 1.1, fontWeight: 900, color: "#64748b" }}>LANDGRID · SETTINGS</div><div style={{ fontSize: 18, fontWeight: 800 }}>{project.name}</div></div></div>
    ...
    <div style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 22px 50px" }}>
      <div style={{ marginBottom: 24 }}><div style={{ fontSize: 11, fontWeight: 900, letterSpacing: 1, color: "#64748b" }}>PROJECT SETTINGS</div><h1 style={{ margin: "5px 0 6px", fontSize: 28, letterSpacing: -.5 }}>Manage your project</h1><p style={{ margin: 0, color: "#64748b", fontSize: 13 }}>Project details and access are managed here.</p></div>
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 340px", gap: 16, alignItems: "start" }}>
        <div style={{ display: "grid", gap: 16 }}>
          ...
          <section style={cardStyle}>
            ...
            <form onSubmit={addMember} style={{ display: "grid", gridTemplateColumns: "1fr 130px auto", gap: 8, marginBottom: 15 }}><input aria-label="User email" value={email} onChange={e => setEmail(e.target.value)} placeholder="name@company.com" type="email" style={inputStyle} /><select aria-label="User role" value={role} onChange={e => setRole(e.target.value as "admin" | "sales")} style={inputStyle}><option value="sales">Sales</option><option value="admin">Admin</option></select><button type="submit" disabled={busy} style={{ ...buttonStyle, background: busy ? "#eef2f7" : "#172554", color: busy ? "#94a3b8" : "#fff", borderColor: busy ? "#dbe2ea" : "#172554" }}>{busy ? "Adding…" : "Add user"}</button></form>
            {message && <div style={{ marginBottom: 12, fontSize: 12, color: message === "User added" ? "#15803d" : "#b45309" }}>{message}</div>}
            <div style={{ display: "grid", gap: 7 }}>{members.map(member => <div key={member.userId} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 12px", border: "1px solid #e8edf3", borderRadius: 10, background: "#fafbfc" }}><div><div style={{ fontSize: 13, fontWeight: 750 }}>{member.name || member.email}</div>{member.name && <div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>{member.email}</div>}</div><span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 10, fontWeight: 900, textTransform: "uppercase", letterSpacing: .6, color: member.role === "admin" ? "#3730a3" : "#475569", background: member.role === "admin" ? "#eef2ff" : "#f1f5f9", padding: "5px 8px", borderRadius: 999 }}><Shield size={11} /> {member.role}</span></div>)}{!members.length && <div style={{ padding: 18, textAlign: "center", border: "1px dashed #dbe2ea", borderRadius: 10, color: "#94a3b8", fontSize: 12 }}>No project users found.</div>}</div>
          </section>
        </div>

        <aside style={{ display: "grid", gap: 12 }}>
          <div style={cardStyle}><div style={{ fontSize: 10, letterSpacing: 1, fontWeight: 900, color: "#64748b" }}>SETTINGS</div><div style={{ marginTop: 12, display: "grid", gap: 4 }}><div style={{ padding: "10px 0", fontSize: 12, fontWeight: 800, color: "#172554", borderBottom: "1px solid #edf1f5" }}>Project details</div><div style={{ padding: "10px 0", fontSize: 12, fontWeight: 800, color: "#172554" }}>Users & access</div></div></div>
