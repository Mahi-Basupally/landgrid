    ...
    <header className="app-header">
      <div className="app-header-left">
        ...
        <nav className="app-nav" aria-label="Primary">
          ...
          {slug && !isEditor && <Link href={`/projects/${encodeURIComponent(slug)}/settings`} className={isSettings ? "app-nav-active" : ""}>SETTINGS</Link>}
        </nav>
      </div>
      <div className="app-header-center" aria-live="polite">
        {displayName && <span className="app-project-name">{displayName}</span>}
        {displayName && !isEditor && <span className="app-heading-separator">-</span>}
        {!isEditor && <span className="app-page-heading">{heading}</span>}
        {isEditor && message ? <span className="app-message" title={message}><MessageSquare size={14} /> {message}</span> : null}
      </div>
