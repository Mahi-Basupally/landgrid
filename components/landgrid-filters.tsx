...
type Lot = { id: string; number: string; ownerId?: string | null; ownerName?: string; area?: number | null; status?: string };
...
function normalizeStatus(value: unknown) {
  ...
  if (status === 'reserved' || status === 'reserve' || status === 'reservation') return 'reserved';
  if (status === 'available' || status === 'avail') return 'available';
  if (status === 'sold' || status === 'sale') return 'sold';
  return status;
}
...
function statusLabel(status: string) {
  ...
  return status ? status.replace(/[_-]+/g, ' ').replace(/\b\w/g, value => value.toUpperCase()) : 'Unknown';
}
...
export default function LandGridFilters({ projectSlug }: { projectSlug: string }) {
  ...
  const [activeTab, setActiveTab] = useState<'owners' | 'plots' | 'summary'>('owners');
    ...
    fetch(`/api/projects/${encodeURIComponent(projectSlug)}/plan`, { cache: 'no-store' })
      ...
      .then(data => {
        ...
        const parsedLots: Lot[] = rawLots.map((lot: any) => {
          ...
          const rawStatus = lot.status ?? lot.plotStatus ?? lot.plot_status ?? lot.saleStatus ?? lot.sale_status ?? '';
          return { id: String(lot.id), number: String(lot.number ?? lot.plot_number ?? ''), ownerId, ownerName: String(lot.ownerName || lot.owner_name || (ownerId ? ownerNames.get(ownerId) : '') || ''), area: lot.area != null ? Number(lot.area) : lot.area_sq_yards != null ? Number(lot.area_sq_yards) : null, status: normalizeStatus(rawStatus) };
        });
  ...
  const filteredPlots = useMemo(() => {
    const query = norm(plotSearch);
    return lots.filter(lot => {
      const matchesSearch = !query || norm(lot.number).includes(query) || norm(lot.ownerName).includes(query);
      const matchesStatus = plotStatus === 'all' || normalizeStatus(lot.status) === plotStatus;
      return matchesSearch && matchesStatus;
    });
  }, [lots, plotSearch, plotStatus]);
  const statusCounts = useMemo(() => new Map(['available', 'reserved', 'sold'].map(status => [status, lots.filter(lot => normalizeStatus(lot.status) === status).length])), [lots]);
  const selectedLots = useMemo(() => lots.filter(lot => {
    const ownerMatch = lot.ownerId != null && selectedOwners.has(String(lot.ownerId));
    const plotMatch = activeTab === 'plots' && filteredPlots.some(item => item.id === lot.id);
    return selectedOwners.size > 0 ? ownerMatch && (plotStatus === 'all' || normalizeStatus(lot.status) === plotStatus) && (!plotSearch || plotMatch) : plotMatch;
  }), [lots, selectedOwners, activeTab, filteredPlots, plotStatus, plotSearch]);
  ...
  const summaryLots = activeSummaryOwner ? lots.filter(lot => lot.ownerId === activeSummaryOwner.id) : lots.filter(lot => lot.ownerId != null && selectedOwners.has(String(lot.ownerId)));
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      svg.querySelectorAll<SVGGElement>('g').forEach(group => {
        ...
        const ownerActive = ownerId !== '' && selectedOwners.has(ownerId);
        const plotActive = activeTab === 'plots' && filteredPlots.some(item => item.id === lot.id);
        const active = activeTab === 'plots' ? plotActive : ownerActive;
        if (active) {
  ...
  }, [lots, selectedOwners, ownerMap, ownerIndex, activeTab, filteredPlots]);
  ...
  return (
    <div className="lg-shell-overlay">
      <button type="button" className="lg-filter-trigger" onClick={() => setMobileOpen(true)} aria-label="Open map filters">
        <SlidersHorizontal size={16} /><strong>Filters</strong>{(selectedOwners.size > 0 || (activeTab === 'plots' && (plotSearch || plotStatus !== 'all'))) && <span className="lg-filter-count">{activeTab === 'plots' ? filteredPlots.length : selectedOwners.size}</span>}
      </button>
      ...
      {mobileOpen && <div className="lg-modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) closeModal(); }}>
        <aside className="lg-filter-modal" role="dialog" aria-modal="true" aria-label="Map filters">
          <header className="lg-modal-header">
            <div><strong>Filters</strong><span>{activeTab === 'plots' ? `${filteredPlots.length} matching plots` : selectedOwners.size ? `${selectedOwners.size} owner${selectedOwners.size === 1 ? '' : 's'} selected · ${selectedLots.length} plots` : 'Choose what to show on the map'}</span></div>
            <div className="lg-header-actions">{(selectedOwners.size > 0 || plotSearch || plotStatus !== 'all') && <button type="button" className="lg-clear" onClick={() => { clearOwners(); setPlotSearch(''); setPlotStatus('all'); }}>Clear</button>}<button type="button" className="lg-close" onClick={closeModal} aria-label="Close filters"><X size={18} /></button></div>
          </header>
          ...
          <nav className="lg-tabs" aria-label="Filter navigation">
            ...
            <button type="button" className={activeTab === 'plots' ? 'active' : ''} onClick={() => setActiveTab('plots')}><span>Plots</span>{lots.length > 0 && <em>{lots.length}</em>}</button>
          ...
          <div className="lg-modal-content">
            ...
            </section> : activeTab === 'plots' ? <section>
              <div className="lg-section-intro"><div><strong>Plot filters</strong><span>Search by plot number or owner, then filter by availability status.</span></div></div>
              <div className="lg-search-wrap"><Search size={15} /><input value={plotSearch} onChange={event => setPlotSearch(event.target.value)} placeholder="Search plot number or owner" aria-label="Search plots" />{plotSearch && <button type="button" aria-label="Clear plot search" onClick={() => setPlotSearch('')}><X size={14} /></button>}</div>
              <div className="lg-plot-filter-row">
                <label htmlFor="lg-plot-status">Status</label>
                <select id="lg-plot-status" value={plotStatus} onChange={event => setPlotStatus(event.target.value)}>
                  <option value="all">All statuses ({lots.length})</option>
                  <option value="available">Available ({statusCounts.get('available') || 0})</option>
                  <option value="reserved">Reserve ({statusCounts.get('reserved') || 0})</option>
                  <option value="sold">Sold ({statusCounts.get('sold') || 0})</option>
                </select>
              </div>
              <div className="lg-plot-list lg-filter-plot-list">{filteredPlots.map(lot => {
                const owner = lot.ownerId ? ownerMap.get(lot.ownerId) : null;
                return <button key={lot.id} type="button" className="lg-plot-filter-item" onClick={() => { setPlotSearch(lot.number); }}>
                  <span><b>Plot {lot.number}</b><small>{lot.ownerName || 'No owner'}{lot.area != null ? ` · ${formatYards(Number(lot.area))} yd²` : ''}</small></span>
                  <strong className={`lg-status lg-status-${normalizeStatus(lot.status)}`}>{statusLabel(normalizeStatus(lot.status))}</strong>
                  {owner && <i className="lg-dot" style={{ background: colorFor(owner, ownerIndex.get(owner.id) ?? 0) }} />}
                </button>;
              })}{filteredPlots.length === 0 && <div className="lg-empty">No plots match the selected filters.</div>}</div>
            </section> : <section>
