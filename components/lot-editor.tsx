...
export default function LotEditor({ projectSlug }: { projectSlug: string }) {
  ...
  function move(e: React.PointerEvent<SVGSVGElement>) {
    ...
    let q = drag.points!.map((p, i) => drag.kind.endsWith("point") ? (i === drag.index ? { x: p.x + dx, y: p.y + dy } : p) : { x: p.x + dx, y: p.y + dy });
  ...
  return <div style={{ height: "100vh", display: "grid", gridTemplateRows: "68px 1fr", background: "#f3f5f8", color: "#172033", fontFamily: "Inter,system-ui,sans-serif" }}>
    <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 18px", background: "white", borderBottom: "1px solid #e2e8f0" }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}><span style={{ fontSize: 12, color: busy ? "#b45309" : "#15803d" }}>{busy ? "Saving…" : message || "All changes saved"}</span><button type="button" onClick={() => void save()} disabled={!dirty || busy}><Save size={14} /> Save</button></div>
    </header>
    <div style={{ display: "grid", gridTemplateColumns: "285px minmax(0,1fr) 310px", minHeight: 0 }}>
      ...
      <aside style={{ background:"white", borderLeft:"1px solid #e2e8f0", padding:15, overflow:"auto" }}>
        {selectedPlot ? <><div style={{ display:"flex", justifyContent:"space-between" }}><div><small style={{ color:"#64748b" }}>SELECTED PLOT</small><h3 style={{ margin:"2px 0" }}>Plot {selectedPlot.number}</h3></div><button type="button" onClick={() => setSelected(null)}><X size={15}/></button></div><label>Plot Number<input value={selectedPlot.number} onChange={e=>updatePlot("number",e.target.value)} style={{width:"100%"}}/></label><label style={{display:"block",marginTop:10}}>Section<select value={selectedPlot.sectionId || ""} onChange={e=>updatePlot("sectionId",e.target.value)} style={{width:"100%"}}><option value="">Master Plan</option>{sectionList.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label><label style={{display:"block",marginTop:10}}>Status<select value={selectedPlot.status} onChange={e=>updatePlot("status",e.target.value)} style={{width:"100%"}}><option>available</option><option>reserved</option><option>sold</option><option>hold</option></select></label><label style={{display:"block",marginTop:10}}>Owner<input value={selectedPlot.owner} onChange={e=>updatePlot("owner",e.target.value)} style={{width:"100%"}}/></label><label style={{display:"block",marginTop:10}}>Price<input value={selectedPlot.price ?? ""} onChange={e=>updatePlot("price",e.target.value)} style={{width:"100%"}}/></label><label style={{display:"block",marginTop:10}}>Area<input value={selectedPlot.area ?? ""} onChange={e=>updatePlot("area",e.target.value)} style={{width:"100%"}}/></label><button type="button" onClick={()=>setConfirm({kind:"plot",id:selectedPlot.id,label:`Plot ${selectedPlot.number}`})} style={{marginTop:18,width:"100%",padding:9,color:"#b91c1c"}}><Trash2 size={14}/> Delete Plot</button></> : selectedSection ? <><div><small style={{color:"#64748b"}}>SELECTED SECTION</small><h3 style={{margin:"2px 0"}}>{section?.name}</h3></div><label style={{display:"block",marginTop:14}}>Section Display Name<input value={section?.name || ""} onChange={e=>renamePlan(selectedSection,e.target.value)} style={{width:"100%"}}/></label><p style={{fontSize:12,color:"#64748b"}}>Drag any of the 8 corner/edge points to freely reshape the section polygon.</p><button type="button" onClick={()=>setConfirm({kind:"section",id:selectedSection,label:section?.name || selectedSection})} style={{marginTop:12,width:"100%",padding:9,color:"#b91c1c"}}><Trash2 size={14}/> Delete Section</button></> : <><small style={{color:"#64748b"}}>MASTER PLAN</small><h3 style={{margin:"2px 0 14px"}}>{master?.name || "Master Plan"}</h3>{master && <label>Master Plan Display Name<input value={master.name} onChange={e=>renamePlan(master.id,e.target.value)} style={{width:"100%"}}/></label>}<p style={{fontSize:12,color:"#64748b",marginTop:14}}>Click empty canvas to deselect. Hold and drag anywhere on the map or drone image to pan the canvas. Plot and section shapes stop the pan and can be edited with 8 points.</p></>}
      </aside>
    </div>
    {confirm && <div style={{position:"fixed",inset:0,zIndex:50,display:"grid",placeItems:"center",background:"rgba(15,23,42,.45)"}}><div style={{width:360,background:"white",borderRadius:12,padding:22}}><h3>Delete {confirm.kind}?</h3><p>Delete <b>{confirm.label}</b>? This cannot be undone.</p><div style={{display:"flex",justifyContent:"flex-end",gap:8}}><button type="button" onClick={()=>setConfirm(null)}>Cancel</button><button type="button" disabled={busy} onClick={()=>confirm.kind === "section" ? void deleteSection(confirm.id) : (setPlots(v=>v.filter(p=>p.id!==confirm.id)), setSelected(null), setConfirm(null), setDirty(true))} style={{background:"#b91c1c",color:"white",border:0,padding:"9px 14px",borderRadius:7}}>Delete</button></div></div></div>}
  </div>;
}
