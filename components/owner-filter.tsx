...
export default function OwnerFilter({ projectSlug }: { projectSlug: string }) {
  ...
  function exportPdfReport() {
    ...
    try {
      const ownerSections = selectedSummary.map(({ owner, ownerLots, totalYards }) => {
        const pal = colorByOwner.get(owner.id) || paletteFor(0);
        const color = esc(pal.base), light = esc(pal.light);
        const plots = ownerLots.slice().sort((a, b) => Number(a.number) - Number(b.number)).map(l => `<div class=\"pdf-plot-chip\"><b>Plot ${esc(l.number)}</b><span> · ${fmtYards(l.area)} yd²</span></div>`).join('');
        return `<section class=\"pdf-owner-card\"><div class=\"pdf-owner-head\" style=\"background:${light};border-color:${light}\"><div class=\"pdf-owner-name\"><span class=\"pdf-dot\" style=\"background:${color}\"></span><b>${esc(owner.name)}</b></div><b class=\"pdf-owner-total\">${ownerLots.length} plots · ${fmtYards(totalYards)} sq.yd</b></div><div class=\"pdf-plot-grid\">${plots}</div></section>`;
      }).join('');
