...
export default function PlotEditor({ projectSlug, onStatusChange }: { projectSlug: string; onStatusChange?: (msg: string) => void }) {
  ...
  function choosePlot(id: string) { if (!id) { setSelected(null); return; }
    setPlots(v => v.map(p => p.id === selected && selected !== id ? { ...p, locked: true } : p)); const p = plots.find(x => x.id === id); if (!p) return; const q = normalize(parse(p.points)), c = center(q), b = bounds(q), target = Math.min(8, Math.max(.75, Math.min(W / Math.max(1, b.maxX - b.minX), H / Math.max(1, b.maxY - b.minY)) * .55)); setSelected(id); setSelectedSection(null); setPlanId(p.sectionId || "master_plan"); setShapeMode(q.length === 4 ? "rectangle" : "polygon"); if (p.locked !== false) { setZoom(target); setPan({ x: c.x - W / 2, y: c.y - H / 2 }); } }
