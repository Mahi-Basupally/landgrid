...
export default function PlotEditor({ projectSlug, onStatusChange }: { projectSlug: string; onStatusChange?: (msg: string) => void }) {
  ...
  function renderPlotAnnotations(q: Point[], p: Plot, z: number) {
    const c = center(q);
    ...
    return <>
      {/* Plot number */}
      <text x={c.x} y={sqYd ? c.y - 18/z : c.y} textAnchor="middle" dominantBaseline="middle" fontSize={22/z} fontWeight={900} pointerEvents="none" fill="#172033" paintOrder="stroke" stroke="white" strokeWidth={5/z}>{p.number}</text>
      {/* Area in sq.yd */}
      {sqYd != null && <text x={c.x} y={c.y + 16/z} textAnchor="middle" dominantBaseline="middle" fontSize={14/z} fontWeight={700} pointerEvents="none" fill="#475569" paintOrder="stroke" stroke="white" strokeWidth={3/z}>{sqYd} sq.yd</text>}
      {/* Edge dimension labels — only when plot is selected and has dims stored */}
      {(lm || wm) && sorted.map((e, i) => {
