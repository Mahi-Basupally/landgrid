...
type Plot = { id: string; number: string; status: string; owner: string; ownerId?: string | null; price: number | string | null; area: number | string | null; areaSqFt: number | null; lengthM: number | null; widthM: number | null; direction: string; model: string; notes: string; points: string; labelX: number; labelY: number; sectionId?: string | null };
...
export default function PlotEditor({ projectSlug, onStatusChange }: { projectSlug: string; onStatusChange?: (msg: string) => void }) {
  ...
  function addPlot() { const n = String(Math.max(0, ...plots.map(p => Number(p.number)).filter(Number.isFinite)) + 1), sectionId = planId === "master_plan" ? null : planId, q = polygon8([{ x: 650, y: 430 }, { x: 830, y: 430 }, { x: 830, y: 540 }, { x: 650, y: 540 }]), c = center(q); const p: Plot = { id: crypto.randomUUID(), number: n, status: "available", owner: "", ownerId: null, price: null, area: null, areaSqFt: null, lengthM: null, widthM: null, direction: "", model: "", notes: "", points: stringify(q), labelX: c.x, labelY: c.y, sectionId }; setPlots(v => [...v, p]); setSelected(p.id); setSelectedSection(null); setShapeMode("polygon"); setDirty(true); setMessage(`Plot ${n} added${sectionId ? ` to ${plans.find(s => s.id === sectionId)?.name || "Section"}` : ""}`); }
  ...
  function plotLabel(p: Plot) { const dims = (p.lengthM && p.widthM) ? `${dimStr(p.lengthM)}×${dimStr(p.widthM)}` : ""; if (labelMode === "dims") return dims || p.number; if (labelMode === "both") return dims ? `${p.number}\n${dims}` : p.number; return p.number; }
