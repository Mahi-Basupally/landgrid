...
import { ChevronDown, ChevronUp, Copy, Download, Eye, EyeOff, FileJson, Menu, Minus, Plus, Save, Trash2, Upload, X, UserPlus } from "lucide-react";
...
type Plot = { id: string; number: string; status: string; ownerId?: string | null; price: number | string | null; area: number | string | null; areaSqFt: number | null; lengthM: number | null; widthM: number | null; direction: string; model: string; notes: string; locked?: boolean; points: string; labelX: number; labelY: number; sectionId?: string | null };
...
export default function PlotEditor({ projectSlug, onStatusChange }: { projectSlug: string; onStatusChange?: (msg: string) => void }) {
  ...
  const [leftOpenM, setLeftOpenM] = useState(false), [detailOpenM, setDetailOpenM] = useState(false);
  ...
  function deselect() {
    setPlots(v => v.map(p => p.id === selected ? { ...p, locked: true } : p));
    setSelected(null); setSelectedIds(new Set()); setSelectedSection(null); setDrag(null);
  }
  ...
  function updatePlot(k: keyof Plot, value: string | number | null) { if (!selectedPlot) return; setPlots(v => v.map(p => p.id === selectedPlot.id ? { ...p, [k]: k === "sectionId" || k === "ownerId" ? (value || null) : value } : p)); setDirty(true); if (k === "ownerId") void saveOwner(selectedPlot.id, value as string | null); }
  async function saveOwner(plotId: string, ownerId: string | null) { try { await fetch(`/api/projects/${encodeURIComponent(projectSlug)}/plots/owner`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ plotId, ownerId: ownerId || null }) }); } catch {} }
  function updateMulti(key: "status" | "sectionId" | "ownerId", value: string) { setPlots(v => v.map(p => selectedIds.has(p.id) ? { ...p, [key]: value || null } : p)); setDirty(true); if (key === "ownerId") { for (const plotId of selectedIds) void saveOwner(plotId, value || null); } }
  ...
  function choosePlot(id: string) { if (!id) { setSelected(null); setDetailOpenM(false); return; }
    setPlots(v => v.map(p => p.id === selected && selected !== id ? { ...p, locked: true } : p)); const p = plots.find(x => x.id === id); if (!p) return; const q = normalize(parse(p.points)), c = center(q); setSelected(id); setSelectedSection(null); setPlanId(p.sectionId || "master_plan"); setShapeMode(q.length === 4 ? "rectangle" : "polygon"); setZoom(1.5); setPan({ x: c.x - W / 2, y: c.y - H / 2 }); }
  ...
  function startCanvasPan(e: React.PointerEvent<SVGSVGElement>) { if (e.button !== 0) return; if (!e.ctrlKey && !e.metaKey) { deselect(); } setDrag({ kind: "pan", id: "canvas", start: { x: e.clientX, y: e.clientY }, panStart: { ...pan } }); e.currentTarget.setPointerCapture(e.pointerId); }
  function startShape(e: React.PointerEvent<SVGElement>, id: string, q: Point[], sectionShape: boolean) { e.stopPropagation(); if (!sectionShape) { const plt = plots.find(p => p.id === id); if (plt?.locked) {
      // Lock previous plot when switching to this locked one
      setPlots(v => v.map(p => p.id === selected && selected !== id ? { ...p, locked: true } : p));
      setSelected(id); setSelectedSection(null); e.currentTarget.setPointerCapture(e.pointerId); return; } } if (sectionShape) { setSelectedSection(id); setSelected(null); setSelectedIds(new Set()); } else { if (e.ctrlKey || e.metaKey) { setSelectedIds(prev => { const next = new Set(prev); if (next.has(id)) { next.delete(id); } else { next.add(id); if (selected) next.add(selected); } return next; }); setSelected(id); setSelectedSection(null); } else { const isInMulti = selectedIds.has(id) && selectedIds.size > 1; if (!isInMulti) {
          // Lock the previously selected plot when switching to a different one
          setPlots(v => v.map(p => p.id === selected && selected !== id ? { ...p, locked: true } : p));
          setSelectedIds(new Set([id])); setSelected(id); } else { setSelected(id); } setSelectedSection(null);

        // zoom into clicked plot only when locked
        const plt2 = plots.find(p => p.id === id); if (plt2?.locked !== false) { const b2 = bounds(q); const target2 = Math.min(6, Math.max(1.5, Math.min(W/(Math.max(1,b2.maxX-b2.minX)*1.5), H/(Math.max(1,b2.maxY-b2.minY)*1.5)))); setZoom(target2); setPan({ x: center(q).x - W/2, y: center(q).y - H/2 }); } } } if (!sectionShape) { const isMulti = (e.ctrlKey || e.metaKey) || (selectedIds.has(id) && selectedIds.size > 1); setShapeMode(q.length === 4 ? "rectangle" : "polygon"); if (isMulti && selectedIds.size > 1) { const allPoints = plots.filter(p => selectedIds.has(p.id)).map(p => ({ id: p.id, pts: normalize(parse(p.points)) })); setDrag({ kind: "multimove", id, start: point(e), points: q, allPoints } as any); } else { setDrag({ kind: "move", id, start: point(e), points: q, section: false }); } } else { setShapeMode(q.length === 4 ? "rectangle" : "polygon"); setDrag({ kind: "move", id, start: point(e), points: q, section: true }); } e.currentTarget.setPointerCapture(e.pointerId); }
  ...
  function exportPlots() {
    const data = plots.map(p => ({
      ...
      ownerId: p.ownerId, direction: p.direction,
  ...
  return <div style={{ height: "100%", display: "grid", gridTemplateRows: "1fr", background: "#f4f6f9", color: "#182235", fontFamily: "Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" }}>
    <style dangerouslySetInnerHTML={{ __html: `
      .pe-grid { display: grid; grid-template-columns: 290px minmax(0,1fr) 330px; min-height: 0; height: 100%; }
      .pe-left { background: #fff; border-right: 1px solid #e2e8f0; padding: 12px; overflow: auto; }
      .pe-right { background: #fff; border-left: 1px solid #e2e8f0; padding: 14px; overflow: auto; }
      .pe-hint { display: inline; }
      .pe-mobile-bar { display: none; }
      .pe-bottom-sheet { display: none; }
      .pe-overlay { display: none; }
      @media (max-width: 767px) {
        .pe-grid { grid-template-columns: minmax(0,1fr); grid-template-rows: 1fr; }
        .pe-left { display: none; }
        .pe-right { display: none; }
        .pe-hint { display: none; }
        .pe-mobile-bar { display: flex; position: absolute; bottom: 0; left: 0; right: 0; z-index: 20; background: rgba(255,255,255,.97); border-top: 1px solid #e2e8f0; padding: 8px 10px; gap: 8px; align-items: center; box-shadow: 0 -4px 16px rgba(15,23,42,.08); padding-bottom: max(8px, env(safe-area-inset-bottom)); }
        .pe-bottom-sheet { display: block; position: fixed; left: 0; right: 0; bottom: 0; z-index: 40; background: #fff; border-radius: 18px 18px 0 0; box-shadow: 0 -8px 32px rgba(15,23,42,.18); max-height: 78vh; overflow-y: auto; transform: translateY(100%); transition: transform .28s cubic-bezier(.4,0,.2,1); }
        .pe-bottom-sheet.open { transform: translateY(0); }
        .pe-sheet-handle { width: 36px; height: 4px; background: #cbd5e1; border-radius: 2px; margin: 10px auto 6px; }
        .pe-overlay { display: block; position: fixed; inset: 0; z-index: 35; background: rgba(15,23,42,.32); opacity: 0; pointer-events: none; transition: opacity .25s; }
        .pe-overlay.open { opacity: 1; pointer-events: auto; }
      `}} />
...
<label style={{ display: "block", marginTop: 11, fontSize: 11, fontWeight: 800, color: selectedPlot.locked ? "#94a3b8" : "#475569" }}>Notes
  <input value={selectedPlot.notes} readOnly={selectedPlot.locked} onChange={e => !selectedPlot.locked && updatePlot("notes", e.target.value)} style={{ ...input, marginTop: 5, background: selectedPlot.locked ? "#f8fafc" : "#fff", color: selectedPlot.locked ? "#94a3b8" : "#172033" }} />
    ...
    {/* Mobile overlay */}
    <div className={`pe-overlay${leftOpenM || detailOpenM ? " open" : ""}`} onClick={() => { setLeftOpenM(false); setDetailOpenM(false); }} />
    {/* Mobile left drawer */}
    <div className={`pe-bottom-sheet${leftOpenM ? " open" : ""}`} style={{ maxHeight: "82vh" }}>
      <div className="pe-sheet-handle" />
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 16px 8px" }}>
        <div style={{ fontSize: 13, fontWeight: 800 }}>Plots & Tools</div>
        <button type="button" onClick={() => setLeftOpenM(false)} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }}><X size={18}/></button>
      </div>
      <div style={{ padding: 12 }}>
        <button type="button" onClick={() => setPlanId("master_plan")} style={{ ...button, width: "100%", justifyContent: "flex-start", marginBottom: 8 }}>▦ Master Plan</button>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 6, marginBottom: 10 }}>
          <button type="button" onClick={() => { addPlot(); setLeftOpenM(false); }} style={{ ...button, background: "#172554", color: "#fff", borderColor: "#172554" }}><Plus size={14}/> Add Plot</button>
          <button type="button" onClick={() => importInput.current?.click()} style={{ ...button, padding: "8px 10px" }}><FileJson size={14}/></button>
          <button type="button" onClick={exportPlots} style={{ ...button, padding: "8px 10px" }}><Download size={14}/></button>
        </div>
        <div style={{ fontSize: 10, fontWeight: 900, color: "#7b8798", margin: "12px 0 6px", letterSpacing: 1 }}>MAP / DRONE</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <button type="button" style={{ ...button, width: "100%" }} onClick={() => mapInput.current?.click()}><Upload size={13}/> Map</button>
          <button type="button" style={{ ...button, width: "100%" }} onClick={() => droneInput.current?.click()}><Upload size={13}/> Drone</button>
        </div>
      </div>
    </div>
    {/* Mobile detail sheet */}
    <div className={`pe-bottom-sheet${detailOpenM && (selectedPlot || selectedIds.size > 1) ? " open" : ""}`}>
      <div className="pe-sheet-handle" />
      <div style={{ padding: "0 14px 24px" }}>
        {/* Same right panel content rendered inline for mobile */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div style={{ fontSize: 12, fontWeight: 900, color: "#64748b" }}>{selectedPlot ? `Plot ${selectedPlot.number}` : selectedIds.size > 1 ? `${selectedIds.size} selected` : ""}</div>
          <button type="button" onClick={() => setDetailOpenM(false)} style={{ ...button, padding: 5 }}><X size={14}/></button>
        </div>
        {selectedPlot && <>
          <div style={{ display: "flex", gap: 8, alignItems: "center", padding: "8px 10px", borderRadius: 8, background: selectedPlot.locked ? "#fff7ed" : "#f0fdf4", border: `1px solid ${selectedPlot.locked ? "#fed7aa" : "#bbf7d0"}`, marginBottom: 10 }}>
            <div style={{ flex: 1, fontSize: 12, fontWeight: 800, color: selectedPlot.locked ? "#9a3412" : "#166534" }}>{selectedPlot.locked ? "🔒 Locked" : "🔓 Unlocked"}</div>
            <button type="button" onClick={() => toggleLock(selectedPlot.id)} style={{ ...button, padding: "5px 10px", background: selectedPlot.locked ? "#fff" : "#16a34a", color: selectedPlot.locked ? "#9a3412" : "#fff", borderColor: selectedPlot.locked ? "#fed7aa" : "#16a34a" }}>{selectedPlot.locked ? "Unlock" : "Lock"}</button>
          </div>
          <div style={{ opacity: selectedPlot.locked ? 0.5 : 1, pointerEvents: selectedPlot.locked ? "none" : "auto", display: "grid", gap: 10 }}>
            <label style={{ fontSize: 11, fontWeight: 800, color: "#475569" }}>Status
              <select value={selectedPlot.status} disabled={selectedPlot.locked} onChange={e => !selectedPlot.locked && updatePlot("status", e.target.value)} style={{ ...input, marginTop: 4 }}><option>available</option><option>reserved</option><option>sold</option><option>hold</option></select>
            </label>
            <label style={{ fontSize: 11, fontWeight: 800, color: "#475569" }}>Owner
              <select value={selectedPlot.ownerId || ""} onChange={e => !selectedPlot.locked && updatePlot("ownerId", e.target.value)} disabled={selectedPlot.locked} style={{ ...input, marginTop: 4 }}><option value="">Unassigned</option>{owners.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}</select>
            </label>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: 6 }}>
              <label style={{ fontSize: 11, fontWeight: 800, color: "#475569" }}>Length<input type="number" min="0" step="0.01" value={selectedPlot.lengthM ?? ""} onChange={e => { const v = parseFloat(e.target.value); if (isNaN(v)) { updatePlot("lengthM", null); return; } updatePlot("lengthM", v); if (selectedPlot.widthM) { const sqm = v * selectedPlot.widthM; updatePlot("area", +(sqm * 1.196).toFixed(2)); updatePlot("areaSqFt", +(sqm * 10.764).toFixed(2)); } }} style={{ ...input, marginTop: 4 }} /></label>
              <label style={{ fontSize: 11, fontWeight: 800, color: "#475569" }}>Width<input type="number" min="0" step="0.01" value={selectedPlot.widthM ?? ""} onChange={e => { const v = parseFloat(e.target.value); if (isNaN(v)) { updatePlot("widthM", null); return; } updatePlot("widthM", v); if (selectedPlot.lengthM) { const sqm = selectedPlot.lengthM * v; updatePlot("area", +(sqm * 1.196).toFixed(2)); updatePlot("areaSqFt", +(sqm * 10.764).toFixed(2)); } }} style={{ ...input, marginTop: 4 }} /></label>
              <span style={{ fontSize: 10, color: "#64748b", alignSelf: "end", paddingBottom: 10 }}>m</span>
            </div>
            {selectedPlot.lengthM && selectedPlot.widthM ? <div style={{ padding: "7px 10px", borderRadius: 8, background: "#f0f9ff", border: "1px solid #bae6fd", fontSize: 12 }}><b>{selectedPlot.area ?? 0} sq.yd</b> · {selectedPlot.areaSqFt ?? 0} sq.ft</div> : null}
            <label style={{ fontSize: 11, fontWeight: 800, color: "#475569" }}>Price<input value={String(selectedPlot.price ?? "")} readOnly={selectedPlot.locked} onChange={e => !selectedPlot.locked && updatePlot("price", e.target.value)} style={{ ...input, marginTop: 4 }} /></label>
            <label style={{ fontSize: 11, fontWeight: 800, color: "#475569" }}>Notes<input value={selectedPlot.notes} readOnly={selectedPlot.locked} onChange={e => !selectedPlot.locked && updatePlot("notes", e.target.value)} style={{ ...input, marginTop: 4 }} /></label>
            {!selectedPlot.locked && <button type="button" onClick={() => { setConfirm({ kind: "plot", id: selectedPlot.id, label: `Plot ${selectedPlot.number}` }); setDetailOpenM(false); }} style={{ ...button, width: "100%", color: "#b91c1c", borderColor: "#fecaca", background: "#fff7f7" }}><Trash2 size={14}/> Delete plot</button>}
          </div>
        </>}
      </div>
    </div>
