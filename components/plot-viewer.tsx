...
export default function PlotViewer({ projectSlug }: { projectSlug: string }) {
  ...
  const [plans, setPlans] = useState<Plan[]>([]), [lots, setLots] = useState<Lot[]>([]);
  const [planId, setPlanId] = useState("master_plan"), [selected, setSelected] = useState<string | null>(null);
  const [zoom, setZoom] = useState(1), [pan, setPan] = useState<Point>({ x: 0, y: 0 });
  const [filterStatus, setFilterStatus] = useState("all"), [filterSearch, setFilterSearch] = useState("");
  ...
  const [unit, setUnit] = useState<"m" | "ft" | "yd">("m"), [loading, setLoading] = useState(true);
  const svgRef = useRef<SVGSVGElement>(null);
  const drag = useRef<{ sx: number; sy: number; px: number; py: number } | null>(null);
  const pinch = useRef<{ dist: number; px: number; py: number; zoom: number } | null>(null);
  const vw = cw / zoom, vh = ch / zoom, viewBox = `${pan.x} ${pan.y} ${vw} ${vh}`;
  ...
  useEffect(() => {
    fetch(`/api/projects/${encodeURIComponent(projectSlug)}/plan`, { cache: "no-store" })
      .then(r => r.json()).then(d => {
        ...
        setCw(W); setCh(H);
        const master = (d.sections || []).find((s: any) => s.id === "master_plan"), geom = master?.layerGeometry;
        const iw = master?.imageWidth ?? null, ih = master?.imageHeight ?? null;
        const dx = iw ? (W - iw) / 2 : 0, dy = ih ? (H - ih) / 2 : 0, dw = iw || W, dh = ih || H;
        if (geom?.map?.width) setMapL(l => ({ ...l, ...geom.map, visible: geom.map.visible !== false })); else setMapL(l => ({ ...l, x: dx, y: dy, width: dw, height: dh, visible: true }));
        if (geom?.drone?.width) setDroneL(l => ({ ...l, ...geom.drone, visible: geom.drone.visible !== false })); else setDroneL(l => ({ ...l, x: dx, y: dy, width: dw, height: dh, visible: true }));
        setPlans(d.sections || []);
        setLots((d.lots || []).map((l: any) => ({ id: l.id, number: l.number, status: l.status || "available", ownerName: l.ownerId ? ((d.owners || []).find((o: any) => o.id === l.ownerId)?.name || null) : null, price: l.price, area: l.area ?? null, areaSqFt: l.areaSqFt ?? null, lengthM: l.lengthM ?? null, widthM: l.widthM ?? null, direction: l.direction || "", notes: l.details || l.notes || "", points: l.points || "", sectionId: l.sectionId || null })));
        // Mobile opens at exactly 100%; Fit is also exactly 100%.
  ...
  function assetUrl(kind: "map" | "drone") {
    ...
    return url ? `/api/projects/${encodeURIComponent(projectSlug)}/assets/file?kind=${kind === "map" ? "master-plan" : "drone"}&planType=master_plan&v=${encodeURIComponent(url)}` : null;
  }
  function fitView() { setZoom(1); setPan({ x: 0, y: 0 }); }
  function focusLot(lot: Lot) {
    const q = normalize(parse(lot.points)), c = center(q);
    setSelected(lot.id); setPlanId(lot.sectionId || "master_plan"); setZoom(1.5); setPan({ x: c.x - cw / 2, y: c.y - ch / 2 });
  }
  ...
  function onTouchStart(e: React.TouchEvent) { if (e.touches.length === 2) { const t0 = e.touches[0], t1 = e.touches[1], dist = Math.hypot(t1.clientX - t0.clientX, t1.clientY - t0.clientY); pinch.current = { dist, px: (t0.clientX + t1.clientX) / 2, py: (t0.clientY + t1.clientY) / 2, zoom }; } }
  function onTouchMove(e: React.TouchEvent) { if (e.touches.length === 2 && pinch.current) { e.preventDefault(); const t0 = e.touches[0], t1 = e.touches[1], dist = Math.hypot(t1.clientX - t0.clientX, t1.clientY - t0.clientY); setZoom(Math.min(8, Math.max(.2, pinch.current.zoom * (dist / pinch.current.dist)))); } }
  function onTouchEnd() { pinch.current = null; }

  function renderAnnotations(q: Point[], lot: Lot, z: number, sel: boolean) {
    const c = center(q), sqYd = lot.area != null ? Number(lot.area) : null, lm = lot.lengthM, wm = lot.widthM;
  ...
  const filterContent = <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
    ...
    <div style={{ padding: 12, borderBottom: "1px solid #e2e8f0" }}><div style={{ fontSize: 10, letterSpacing: 1, fontWeight: 900, color: "#7b8798", marginBottom: 7 }}>STATUS</div>{[["all", "All"], ...Object.keys(STATUS).map(k => [k, STATUS[k].label])].map(([key, label]) => <button key={key} onClick={() => setFilterStatus(key)} style={{ ...btn, width: "100%", justifyContent: "space-between", marginBottom: 4, background: filterStatus === key ? "#172554" : "#fff", color: filterStatus === key ? "#fff" : "#243047" }}><span>{label}</span><span style={{ fontSize: 11, opacity: .7 }}>{statusCounts[key] ?? 0}</span></button>)}</div>
    <div style={{ padding: 12, borderBottom: "1px solid #e2e8f0" }}><div style={{ position: "relative" }}><Search size={13} style={{ position: "absolute", left: 9, top: "50%", transform: "translateY(-50%)", color: "#94a3b8" }} /><input value={filterSearch} onChange={e => setFilterSearch(e.target.value)} placeholder="Search plot or owner…" style={{ width: "100%", boxSizing: "border-box", paddingLeft: 28, paddingRight: filterSearch ? 28 : 10, paddingTop: 8, paddingBottom: 8, border: "1px solid #dbe2ea", borderRadius: 8, fontSize: 12, outline: "none" }} />{filterSearch && <button onClick={() => setFilterSearch("")} style={{ position: "absolute", right: 6, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#94a3b8" }}><X size={12} /></button>}</div></div>
    <div style={{ flex: 1, overflowY: "auto", padding: 8 }}>{filtered.length === 0 ? <div style={{ padding: 16, textAlign: "center", color: "#94a3b8", fontSize: 12 }}>No plots match</div> : filtered.slice().sort((a, b) => Number(a.number) - Number(b.number)).map(lot => { const col = sc(lot.status); return <button key={lot.id} onClick={() => focusLot(lot)} style={{ width: "100%", textAlign: "left", padding: "10px", border: `1px solid ${selected === lot.id ? col.dot : "#e4e9f0"}`, borderRadius: 8, background: selected === lot.id ? `${col.dot}18` : "#fff", marginBottom: 5, cursor: "pointer", display: "flex", alignItems: "center", gap: 9 }}><span style={{ width: 9, height: 9, borderRadius: 999, background: col.dot }} /><div><div style={{ fontWeight: 800, fontSize: 13 }}>Plot {lot.number}</div><div style={{ fontSize: 11, color: "#64748b" }}>{col.label}{lot.area != null ? ` · ${lot.area} sq.yd` : ""}</div></div></button>; })}</div>
  </div>;

  const selectedDetail = selectedLot ? <div style={{ padding: "4px 16px 32px" }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}><div><div style={{ fontSize: 10, letterSpacing: 1, fontWeight: 900, color: "#64748b" }}>PLOT DETAILS</div><h3 style={{ margin: "4px 0 0", fontSize: 24 }}>Plot {selectedLot.number}</h3></div><button onClick={() => setSelected(null)} style={{ ...btn, padding: 7 }}><X size={15} /></button></div><div style={{ display: "grid", gap: 12 }}>{[["Status", sc(selectedLot.status).label], ["Owner", selectedLot.ownerName || "Unassigned"], ["Area", selectedLot.area != null ? `${selectedLot.area} sq.yd` : null], ["Price", selectedLot.price != null ? `₹ ${selectedLot.price}` : null], ["Direction", selectedLot.direction || null]].filter(([,v]) => v).map(([label,value]) => <div key={label as string} style={{ padding: "10px 12px", borderRadius: 8, background: "#f8fafc", border: "1px solid #e8edf3" }}><div style={{ fontSize: 10, fontWeight: 900, color: "#94a3b8" }}>{label as string}</div><div style={{ fontSize: 14, fontWeight: 700, marginTop: 3 }}>{value as string}</div></div>)}</div><div style={{ marginTop: 20 }}><select value={unit} onChange={e => setUnit(e.target.value as "m" | "ft" | "yd")} style={{ fontSize: 11, border: "1px solid #e2e8f0", borderRadius: 6, padding: "4px 6px" }}><option value="m">m</option><option value="ft">ft</option><option value="yd">yd</option></select>{(selectedLot.lengthM || selectedLot.widthM) && <div style={{ marginTop: 8, fontWeight: 700 }}>{selectedLot.lengthM ? conv(selectedLot.lengthM) : "—"}{unit} × {selectedLot.widthM ? conv(selectedLot.widthM) : "—"}{unit}</div>}</div></div> : null;

  return <>
    <style>{`*{box-sizing:border-box}.pv{height:100%;flex:1;min-height:0;display:grid;grid-template-columns:260px 1fr 300px;font-family:Inter,ui-sans-serif,system-ui,sans-serif;font-size:13px;color:#182235;background:#f4f6f9}.pv-left{background:#fff;border-right:1px solid #e2e8f0;display:flex;flex-direction:column;overflow:hidden}.pv-canvas{position:relative;overflow:hidden;background:#d9dee5;min-height:0}.pv-right{background:#fff;border-left:1px solid #e2e8f0;overflow-y:auto}.pv-toolbar{position:absolute;top:12px;left:12px;right:12px;z-index:10;display:flex;align-items:center;gap:6px;padding:7px 10px;border:1px solid rgba(226,232,240,.9);border-radius:12px;background:rgba(255,255,255,.97);box-shadow:0 4px 20px rgba(15,23,42,.1)}.pv-hint{font-size:11px;color:#64748b;margin-left:auto}@media(max-width:767px){.pv{grid-template-columns:1fr}.pv-left,.pv-right{display:none}.pv-hint{display:none}.pv-toolbar{top:10px;left:10px;right:10px;padding:6px 8px;gap:4px}.lg-viewer-mobile-clean{display:none}}`}</style>
    <div className="pv">
      ...
      <main className="pv-canvas">
        ...
        <svg ref={svgRef} viewBox={viewBox} preserveAspectRatio="xMidYMid meet" onPointerDown={onPD} onPointerMove={onPM} onPointerUp={onPU} onPointerCancel={onPU} onWheel={onWheel} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", touchAction: "pan-x pan-y pinch-zoom", cursor: "grab" }}>
          ...
          {planLots.map(lot => { const q = normalize(parse(lot.points)), isSel = selected === lot.id, isFilt = !filteredSet.has(lot.id) && (filterStatus !== "all" || filterSearch.trim()), col = sc(lot.status), c = center(q), fillColor = isSel ? "rgba(255,215,0,.22)" : isFilt ? "rgba(255,255,255,.06)" : "rgba(255,255,255,.18)", strokeColor = isSel ? "rgba(218,165,32,.85)" : "transparent", strokeW = isSel ? 3 : 0, dotR = Math.max(4, Math.min(10, 60 / zoom)); return <g key={lot.id} onPointerDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); focusLot(lot); }} style={{ cursor: "pointer" }} opacity={isFilt ? .35 : 1}><polygon points={stringify(q)} fill={fillColor} stroke={strokeColor} strokeWidth={strokeW / zoom} style={{ backdropFilter: "blur(2px)" }} />{isSel && <polygon points={stringify(q)} fill="none" stroke="rgba(255,200,0,.35)" strokeWidth={6 / zoom} />}{!isFilt && !isSel && <circle cx={c.x} cy={c.y} r={dotR} fill={col.dot} opacity={.85} />}{isSel && <circle cx={c.x} cy={c.y} r={dotR * 1.3} fill="rgba(255,215,0,.9)" stroke="rgba(218,165,32,1)" strokeWidth={1.5 / zoom} />}{renderAnnotations(q, lot, zoom, isSel)}</g>; })}
        </svg>
      ...
      <aside className="pv-right">{selectedDetail ?? <div style={{ padding: 16, color: "#64748b" }}>Tap a plot to view details.</div>}</aside>
    </div>
  </>;
}
